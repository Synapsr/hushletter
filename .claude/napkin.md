...
| 2026-02-14 | self | Unit tests for components using TanStack Router `<Link>` crashed without a `RouterProvider` (`useLinkProps` reading `isServer`) | Mock `Link` in unit tests (or wrap renders in a minimal `RouterProvider`) |
| 2026-02-14 | self | Clipboard-dependent UI tests failed because `navigator.clipboard` can be `undefined` in jsdom, so calling `writeText` threw | In tests, define `navigator.clipboard` via `Object.defineProperty(navigator, "clipboard", { value: { writeText: vi.fn() }, configurable: true })` before clicking |
...
| 2026-02-14 | self | Tried calling an exported `internalMutation` function directly from inside another mutation handler | In Convex, call shared logic as a plain helper function, or re-fetch and patch within the current handler; use `ctx.runMutation` only from actions/httpActions, not as direct function calls |
...
| 2026-02-13 | self | Bun runtime workaround conflicted with request to follow TanStack Node+Docker guide | Use Node runtime in Docker (`node /app/apps/web/.output/server/index.mjs`) and polyfill `URLPattern` in app server entry (`src/server.ts`) for portability |
