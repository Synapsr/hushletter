...
function SenderDetailPage() {
  ...
  const newsletters =
    newslettersData?.pages.flatMap((page: NewslettersResponse) => page.items) ?? [];
    ...
    <div className="container mx-auto py-6 px-4 max-w-4xl">
      ...
      <div className="mb-6">
        <div className="flex items-start justify-between gap-4">
          <div>
            <h1 className="text-3xl font-bold text-foreground">{senderData.displayName}</h1>
            {senderData.name && <p className="text-muted-foreground mt-1">{senderData.email}</p>}
          </div>
          {/* Story 6.4 Task 1.5, 2.4: Follow button */}
          <FollowButton senderEmail={senderData.email} senderName={senderData.displayName} />
        </div>
